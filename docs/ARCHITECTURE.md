...
Inspection also moves the decision earlier. For a *not-yet-approved* host, rather than surfacing only the bare CONNECT target (which carries no payload yet), the proxy terminates the cage's TLS, reads the request whole, and fails that attempt without forwarding, so the operator weighs the actual request, not just the host, at approval. A `run`/`call` sees it inline at the prompt; a served run surfaces the captured request out of band (`egress preview`). On approval the caller's next attempt reaches the host, now inspected; nothing reaches it before the decision, and a denial sends nothing. Capturing the request whole means the proxy (the TCB) holds its plaintext before the decision, but it is never forwarded and never written to a shared surface; the body reaches the operator only over the loopback control pull.
